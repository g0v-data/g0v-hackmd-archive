...
/// \return true if the value is present, false if it is not
bool set_contains(const set_t * set, double value) {
    if(set->cmp_fun(set->data[set_index_of(set,value)],value) == 0)
       return true;
    return false;
}
```
...
Add:
The time complexity is O(n) while it has to move all the elements when it adds a new one.

Remove:
The time complexity is O(n) while it has to move all the elements when it removes an element.

Contains:
The time complexity is O(log n) while it use the binary search method to find the index of a value in the sorted array. The binary search has time complexity of O(log n).
