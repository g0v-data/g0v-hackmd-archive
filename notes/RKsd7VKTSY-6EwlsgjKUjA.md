...
As seen from the GIF above, after applying the `useEffect` hook, the autofocus is now implemented. In a React component, it is easy to implement autofocus using `useRef` and `useEffect` together. The `useRef` hook allows us to interact with the `DOM` elements directly while using `useEffect` ensures focus happens after the component is drawn. Therefore, this technique is neat, explicit, and effective, using the potentiality of React Hooks to enhance how users interact with websites.
