...
void array_reserve(array_t *p_array, size_t min_capacity) {
    ...
    if (capacity != p_array->capacity) {
        float * tmp = (float *) realloc(p_array->data, sizeof(size_t[capacity]));
        if (tmp ! = NULL){
            p_array->data = tmp;
...
Answer 2: 
The average case is O(1) and the worst case is O(n). The difference between the two is that the worse case need more time to rellocate the whole array instead of the average case that needs less time while is between the best and worst case.
...
void array_append(array_t *p_array, float value){
    ...
    if(p_array->count > p_array->capacity){
        array_reserve(p_array, p_array->count);
    }
    p_array->data[p_array->count - 1] = value;
}
...
float average(const array_t * grades){
    float sum = 0;
    for(size_t i = 0; i < grades->count; i++)
        sum += grades->data[i];
    size_t aver = sum / (float)grades->count;
    return aver;
}
