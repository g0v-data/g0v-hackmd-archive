...
**1. 志願者 (提供素材以產生一頁式說明)**
我想人工收集並提交素材：提供平台議題資料來源，若湊滿三位志願者相同來源，可讓平台 AI 自動生成一頁式說明

**2. 參與者 (對一頁式說明回饋補充論點)**
我想下載專屬討論包：取得可另外匯給自己 AI 進行討論的檔案 (OPINION.md)
我想回傳意見摘要：將跟自己 AI 討論過的論點摘要，回傳給平台更新議題數據

**3. 公開訪客**
我想查看議題現況：透過主動搜尋或平台熱度排名，快速選擇感興趣的議題查看一頁式說明
...
### 更新: 5/24 與 Ronny 討論 Civic-Talk 重點整理


PoC 初期功能規劃：
* 平台志願參與者只需串 Google 帳號登入平台。
* 「立場地圖」僅是 AI 生成的詞語，尚未有明確定案。可能類似 Polis 的視覺化，也有可能是多張 X-Y 軸光譜分布圖，這部分可以開發過程再討論。
* 在第 1~3 步驟 「*三人貼相同的頁面原文 → 用 AI 整理出共識爭點等項目 → 生成一頁式情境說明*」 ，完全交由 **平台提供 Prompt + 使用者自己的 AI Token 來生成**，平台只是用來公開展示結果，分散成本避免平台負擔過重。（Ronny預估平台維運費用每月僅需數百元，最小化成本不用找贊助也行）
* 上述 AI 應該要用 Agent 型 AI (如 Claude) 進行整理，而非 Chat 型 AI。因為 Agent 型 AI 有三大優點：
    1. Agent 型 AI 可以讀取指定資料夾內容作為參考資料，避免文本長度過長導致忘記前文 or 過度壓縮。
    2. Agent 型 AI 若需要做數值處理計算，可以調用程式來進行實際運算，而非像 Chat 型 AI 是用純機率生成數學式文字。
    3. Agent 型 AI 因為有參考資料，可以根據參考資料作答覆，降低 AI 幻覺亂回答的風險。
* 在 PoC 階段，第 1~3 步驟可以僅由已信任的核心開發者來做資料上傳。到了實際上線階段，會需要再設計 **信任分數機制：初始志願參與者會有基本的信任分數，若後續參與有正常貢獻會提高分數，反之若亂傳資料搞破壞則降低分數。通過信任分數門檻的志願參與者，提交的資料會被平台納入展示，以此來濾掉搞破壞的網軍。**
* PoC 開發階段可以先用 各方利害關係人好找、論述較單純 的議題來做測試 (如 三班醫護比) ，測試 OK 再去考慮更複雜的議題。
* 事實查核可能會在流程中後段再處理。對新聞媒體或 KOL 而言相較於使用嚴重錯假資訊，更常見是僅陳述偏頗資訊 (只講好處不講壞處，反之亦然)，這個平台希望做到的是整合正反方論述，做到完整並陳。
### 0527 討論
- 如果要在審議大會或實體活動時使用這個工具，我們可以如何使用？
    - 一樣可以用這個平台彙整資料，