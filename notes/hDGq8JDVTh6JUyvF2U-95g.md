---
title: "立法院風雲榜 - 專案介紹"
tags: hackpad
---

> [點此觀看原始內容](https://g0v.hackpad.tw/sVOmEE2o0v0)

專案介紹 | Project Readme / Meta

## 來龍去脈（想解決什麼問題）


靈感來源
> 怎麼想到要做這個東西
> [name=ET B]

Mike 分享國外如何促使民意代表之類的通過對人權有助益的法案，其中一個方法就是對議員打分數 A-F，一方面交給大眾，亦促使議員們提升他們分數。

主旨
> 想用什麼方式達成目標
> [name=ET B]

藉由一個網站，利用法條分析、多方面評鑑立委，讓大眾得知立委對各議題的立場、態度，進一步讓大眾評鑑法條，藉此對抗不合理的法條與不適任的立委。

牽涉領域
> e.g.教育 / 環保 / 醫療 / 社福 / 核電 / 都市計畫 / 勞工權益 / 食品安全 ...（會變成專案 tag）
> [name=ET B]


現有類似專案
> 現成的是否可以直接使用？或者有什麼不足之處？國外是否有類似的專案可參考？
> [name=Chia-liang K]

- 美國議員評鑑 [http://thehill.com/resources/lawmaker-ratings#senate](http://thehill.com/resources/lawmaker-ratings#senate)
- 美國議員 ranking [http://www.ranker.com/list/rank-the-current-u-s-senators/sesel88?format=GRID](http://www.ranker.com/list/rank-the-current-u-s-senators/sesel88?format=GRID)
- food議題評鑑 [http://www.foodpolicyaction.org/](http://www.foodpolicyaction.org/)
- 槍械議題評鑑 [http://www.nytimes.com/interactive/2012/12/19/us/politics/nra.html?_r=1&](http://www.nytimes.com/interactive/2012/12/19/us/politics/nra.html?_r=1&)
- 公督盟評鑑 [http://www.ccw.org.tw/assess/%E7%AC%AC%E5%85%AB%E5%B1%86](http://www.ccw.org.tw/assess/%E7%AC%AC%E5%85%AB%E5%B1%86)

相關組織單位
> e.g.公督盟/綠盟/司改會/苦勞網/泛科學...（會變成專案 tag）
> [name=ET B]

- 公督盟

相關專案
> 衍生自某專案/衍生出某專案/API串接自某專案...etc.
> [name=ET B]

- 立法院專案
- 議題與法條相關分析 [http://www.publicwhip.org.uk/index.php](http://www.publicwhip.org.uk/index.php)
- 立場分析 [http://ivoter.ecrc.nsysu.edu.tw/](http://ivoter.ecrc.nsysu.edu.tw/)

授權方式
> 程式碼部分（如 MIT/BSD）
> [name=ET B]

> 文件部分（如 CC-BY）
> [name=ET B]

CC-BY

使用資料

專案目前狀態
> 構想 / 規劃 / 雛形 / 實作
> [name=ET B]

構想
簡報: [https://docs.google.com/presentation/d/1U0G2_RFjAMZTLJViom00hRrh0JlIOFSXp6EGmwlMkW4/pub?start=false&loop=false&delayms=3000](https://docs.google.com/presentation/d/1U0G2_RFjAMZTLJViom00hRrh0JlIOFSXp6EGmwlMkW4/pub?start=false&loop=false&delayms=3000)

## 目標與功能（要如何解決）


預定使用者
公民

預定功能
第一階段：
1.  客觀數據評鑑立委 ex. 出席率、投票率。或者使用公督盟的[評鑑資料](http://www.ccw.org.tw/assess/%E7%AC%AC%E5%85%AB%E5%B1%86)
2.  對法條做分析，法條對應議題的正反面 ex. [http://www.publicwhip.org.uk/index.php](http://www.publicwhip.org.uk/index.php)
3.  法條評鑑系統。 (ex. 支持、不支持、不了解)
第二階段：
1.  在各議題 邀請有公信力團體位立委做評鑑。
2.  針對立委的法條支持與否，得知立委對議題的立場。

使用方式
可以很輕易得知立委各種面相的分數與各議題相關的法條。



## 實做細節（非技術背景可跳填）


使用技術
- 使用的 vagrant box 版本 / 網址：

協作工具
- github repo：
- hackfoldr 工作資料夾網址：
- google drive 共用資料夾網址：
- google groups 討論群組：
- irc channel：

產出檔案格式

進度與 to-do
> 僅供參考
> [name=ET B]

- [ ] product planning（recommanded procedure from justin lee / 李易修）
    - [ ] strategy
    - [ ] scope
    - [ ] structure
    - [ ] wireframe
    - [ ] visual
- [ ] web front-end
- [ ] web back-end
- [ ] ui / visual design


## 成果展示（規劃文件、雛形/草稿、原型/初稿、正式發佈/完稿）


初版 Mockup [https://moqups.com/jessy1092/zANyHHjz](https://moqups.com/jessy1092/zANyHHjz)


## 開發者


技術指導

分工與成員
- [ ] NeedsWriter: 需要文案幫手（撰寫基本資訊、報導專案etc）
- [ ] NeedsDesigner: 需要介面設計
- [ ] NeedsData: 需要資料（擷取、清理）
- [ ] NeedsTech: Lee 需要技術支援（程式、架站 etc)
- [ ] NeedsProcess: 需要幫忙設計作業流程
- [ ] NeedsTalkingToRealPerson: 需要有人幫忙和其他機關聯絡
> 設定成 project registry 中的 tags，需要幫忙的會自動 promote 到 g0v project hub
> [name=ET B]



