...
It is also worth to note that given the capacity for flawless calculation and ability to strictly follow a set command, means that a well-built languagemodel can complete complex calculations or tasks involving copius amounts of numbers under different categories, for example. 
...
1. Manufacturing:
	...
	4. overall output: ai-operated manufacturing facilities can function uninterruptedly day andd night, with consistent and efficient results, which results in a greater quantity of goods within a shorter time 
3. Healthcare: AI for Diagnosing and Planning Treatments
    1. data searching: ai offers an opportunity to keep an ever-expanding mass of knowledge, whilst making it easy and accessible for doctors to find relevant materials to keep themselves up-to-date or compensate for gaps in their expertise 
    2. improved use of bodily scanning tech: currently, machines used for internal body imaginery such as MRI's or PET scans require a specially trained employee in order to interpret the results and operate them, which in turn places restriction onto what the machine can display in order for the result to remain coherent to a human being. WIth the use of an algorithm to process the results and operate the scanners, much more fine details may be acquired. As an example, one of the recent breakthroughs was the usage of an ai exammination on an mri scan of the breast tissue, where it managed to find cancerous cells in the earliest stages of development, which were missed by all human doctors that have analysed it.
5. Finance: AI for Trading and Risk Assessment
6. Customer Service: Chatbots Instead of Human Agents
