...
isalpha(int c):
Checks whether the given character c is an alphabetic character (a-z or A-Z) and returns a non-zero value if c is an alphabet, otherwise returns 0.

isupper(int c):
Checks whether the given character c is an uppercase alphabetic character (A-Z) and returns a non-zero value if c is an uppercase letter, otherwise returns 0.

islower(int c):
Checks whether the given character c is a lowercase alphabetic character (a-z) and returns a non-zero value if c is a lowercase letter, otherwise returns 0.

toupper(int c):
Converts the given lowercase character c to its corresponding uppercase character. If c is already an uppercase letter or not an alphabet, it is returned unchanged.

tolower(int c): 
Converts the given uppercase character c to its corresponding lowercase character. If c is already a lowercase letter or not an alphabet, it is returned unchanged.
