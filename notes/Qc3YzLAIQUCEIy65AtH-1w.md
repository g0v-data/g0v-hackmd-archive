...
* 特色 : 
    1. 此程式在上一組到這一組(選擇模組，並顯示在LED上)的動作的間隔很短，造成模組會被快速刷新。
    2. 模式刷新會增加gHue的值。
...
**模組特色 :**
* **rainbow** : 
    1. 第一顆LED會顯示gHue數值的色調，後一個的**LED色調會+7**，**以此類推**到最後一個LED。
    2. 由於模式刷新會增加gHue的值，**所有LED的顏色會一起變化**。
* **rianbowWithGlitter** : 
    1. 保留rainbow模式的同時。
    2. 在隨機位置上用**白色**覆蓋，因為模式**快速刷新**，看起來像是**閃光**。
* **confetti** : 
    1. 有函式可以讓所有LED都**變暗些許**，在模組刷新速度快的情況下，如果無顏色填充，燈條上的LED都會熄滅。
    2. 顏色填充，但**位置隨機**(被限制在燈條上)。
    3. **色調**被限制在一定範圍內**隨機**(gHue+random8(64))。
