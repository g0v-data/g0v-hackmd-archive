...
---
:::spoiler How did we get here?
```graphviz
digraph hierarchy {

node [fontname=Courier,shape=box]

start [label="Contact tracing?"]
end [label="End"]
all [label="Contact tracing 24/7?"]
fai [label="Fail"]
g0v [label="Decentralised encrypted anonymous\ncontact tracing"]
ano [label="Anonymous?"]
ens [label="Exposure Notifications"]
ensd [label="1. Voluntary basis\n2. Direct contact only\n3. No detailed timeline\n4. May not be 24/7"]
int [label="Online?"]
pap [label="Paper"]
...
sms [label="1922 SMS"]
enc [label="Servers:\nEncrypted user info?"]
clo [label="Closed sources:\ne.g. COVIDPuncher, Ragic"]
ope [label="Open sources:\ne.g. FunTuan, mutix"]
mem [label="Membership:\ne.g. PX Mart, FamilyMart"]
web [label="Web forms:\ne.g. Google Sheets, SurveyCake"]
fourw [label="28 days data delection"]
unk [label="unknown"]
...
int -> pap [label="No"]
pap -> papd
tel -> isp [label="tracking"]
tel -> sms [label="SMS\ntracking"]
isp -> enc [label="tracking"]
enc -> ope [label="Yes"]
enc -> clo [label="No"]
enc -> mem [label="No"]
enc -> web [label="No"]
