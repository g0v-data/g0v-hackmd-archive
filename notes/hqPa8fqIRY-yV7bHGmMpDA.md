...
Each time there is an attempt to refresh, the `handleRefresh` function will be triggered. This function is designed to get a fresh quote using `fetchQuote` and alters the background color using `changeBackgroundColor`. The `changeBackgroundColor` method picks one random color among pre-defined colors structured as Tailwind CSS classes and assigns that color to the background state.
