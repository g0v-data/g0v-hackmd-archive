---
tags: vTaiwan小松
---
# 20250402 vTaiwan 小黑客松
時間 Time ：19:00-20:00
地點 Location ：線上 Online
參與者 Participants: Peter, Josh, Yuting, Allison, Allen, T, Joey 翊婷
線上參與連結 / Link：https://meet.jit.si/vtaiwan

![](https://g0v.hackmd.io/_uploads/HJZ-ajqayx.png)


## 自我介紹與新手導覽
- [新手簡報](https://docs.google.com/presentation/d/1ELAVIpaPVCmAx7nq-7e8-SVrckZ4ohRwn3V-TSpe78U/edit?usp=sharing)
### 自我介紹

## 小小的分享
- 我們要進軍監察院啦！[3/31 監察院國家人權委員會分眾座談](/sfNZiShNRZCXURBF3ueVUg)
    - 目前了解這個討論方式的做法還是透過報告
    - read the room 的討論方法可以寫成專門的介紹文章 [name=josh]
        - 同意 [name=peter]
        - 也可以放在新的網站上 [name=josh]
- 豆腐的數位皮夾志工卡嘗試
    - 看看要不要寫個完整的共筆來介紹與討論
    - [數位皮夾網站](https://wallet.gov.tw/)
        - 感覺可以跟紀念品與尾牙參與資格結合XD [name=peter]
- 議題小聚最新進度

## 進軍監察院
### Todo list
- 索取照片
- 社群媒體上發佈
    - facebook [name=peter]
    - twitter [name=josh]
- 整理成文章
    - 從 12/20 整理出一篇文章，大家幫忙 revise [name=peter]
        - 可以投到 ISF Voice 上 [name=peter]

## 5/23 議題小聚
- [vTaiwan 5/23 議題小聚](/1ZFSjEsIT3mOCgbFk2fo5w)
- [議題手冊](https://docs.google.com/document/d/1yd3Pl1Q5UHWwiFCgt0EylMNnh7Xao5c5re8UOGl8NWk/edit?usp=sharing)
- 主視覺
- [polis 連結](https://polis.tw/87nnxrrj5d)
- 招募表單
- 散布方式
### 討論
- 目前內文文字的敘述，是否要放比較多的說明 [name=josh]
    - 目前已經調整 [name=peter]
- 種子意見可以透過 AI 整理，讓說服力或者是其他條件相同 [name=josh]
    - 另一個可以加上 AI 的方向
    - 一開始生成其實也可以交給 AI，看看哪個時機點可以由 AI 介入。[name=T]
    - 如果要讓 AI supported 的架構有標準化程序的話，可以有一個處理過的方向
        - 有沒有建議的 prompt? 
            - 有模糊的詞義解釋清楚
            - 改寫成三句以內的話語
        - 這些意見可以紀錄下來
### Action list 
- 調整 polis 的內文說明
    - 已經調整好了 [name=peter]
- 調整[種子意見](https://docs.google.com/spreadsheets/d/1xXjnptMcKItVa_9o83R2gnxp-zDwliWnJjNMCK3mW18/edit?gid=0#gid=0)  [name=T] [name=josh]
    - 感覺下週也可以分享 [name=peter]

## 12/20 議題小聚報告討論與修改
- [議題小聚#2「台灣的人工智慧應該如何規範？」報告書](https://docs.google.com/document/d/1kYIYbHdTctGBXrV-U3qivkzByV-uAxOAiS7fB_sGacU/edit?usp=sharing)
    - 感謝 josh、翊婷、T、承哲！

## 主視覺更新
- 之後要統一統一整理到 [vTaiwan 主視覺更新](/UqGh6pAUTwuNFr1RE21Rtw)
- 目前已經大致完成：[figma 連結](https://www.figma.com/design/KrLf8HnypqWKGQUHaQxH67/Tofus-and-his-g0v-graphics!?node-id=213-59)
- 素材要放在哪裡？使用限制？
    - 開一個新的檔案，放到 figma community
    - 預設 CC BY-4.0 
    - 字體：noto serif TC 思源宋體
    - 顏色：
        - 000000
        - 008888
        - D80000
        - DB7700
- 如何維護與授權
    - 雲端硬碟資料夾
    - 素材庫（連結待補）
### 發文分享
- 社群可以先換，舊網站比較麻煩
- 社群發文文字
    - 結合投票章是很好發揮的敘事 [name=josh]
        - 同意 [name=peter]

:::success
我們終於重新設計好了 vTaiwan 的 logo! 這個 logo 結合投票章與反戰符號，同時也結合 vTaiwan 的 V T 與 W 字母意象，以及線條構築出的圓餅圖圖樣，象徵不同意見的比例與匯集。今天也在小松時進行最後的修正與討論。歡迎大家前往以下網址看看
:::

- 圖片
    - 在 figma 上做好圖案
    - 照片可以用 AI 生成蓋投票章在手上的圖片，請 AI 算圖
### Action list 
- 文字 [name=peter] 
- 英文介紹 [name=josh]
- 圖片 [name=tofus]
- figma 稿 [name=tofus]
- 發文 [name=peter] [name=josh]

## 網站更新
- 網站更新
- 可以使用 info.vtaiwan.tw 進行更新
- 目前是使用 zeabur 
- [目前成果影片](https://youtu.be/lb7OVJtkku4?si=64laepxixyWjh__l)
- domain 好像是在 au 手上？[name=peter]
:::success
資金池有得到錢錢 4136 元～恭喜 tofus!!!!
:::
### Action List
- CMS 的問題 [name=tofus]
- 上線 
- 等完整的 demo 完成後，來約au 討論網域，預計四月第三周到第四週 [name=peter]

## Argdown 推廣
- https://allenlinli.notion.site/Argdown-OR-Kialoly-17f028ed171780959f60f98de6e43575?pvs=4
- 感覺可以丟到 g0v 社群上來做討論 [name=peter]
- 或者是可以放到 threads 上來參與討論！[name=josh]
- 兩個可以是平行的，可以用視覺化的方式來做編排 [name=T]
    - 理想上這些討論，有沒有辦法用插件的方式，可以來進行討論
    - 