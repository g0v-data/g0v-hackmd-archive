...
1: The time complexity is O(n) because the program oges from the head to the tail to find the last node.
2: If we would keep track of the tail using a pointer that would reduce our time complexity to O(1), because we know where the tail is so we just update the next pointer of the current tail node to the new node, and then updating the tale pointer to the new node.
...
1: The time complexity is O(n^2) because we go through 2 for loops.
