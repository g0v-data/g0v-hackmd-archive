...
bintree_node *bintree::balanced_insert(bintree_node *root, int value) {
    // TODO:
    //  Insert the value into the tree rooted at root, updating the heights of its nodes while doing so.
    //  If the tree is unbalanced after the insertion, then rebalance it.
...
bintree_node *bintree::balanced_remove(bintree_node *root, int value) {
    if (value == root->m_value) {
        if (root->is_full()) {
            ...
            // TODO: remove the node root, and replace it with its only child (if it has one), decrease count
            bintree_node *tempRoot = root;
        ...
        // TODO: remove the value from the left subtree of root
        root->m_left = balanced_remove(root->m_left, value);
    }
    else if (value > root->m_value && root->has_right_child()) {
        // TODO: remove the value from the right subtree of root
