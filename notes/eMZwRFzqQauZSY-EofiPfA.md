---
tags: vTaiwan
---
# 20260610 小松

時間 Time ：19:00 - 20:00
地點 Location ：線上 Online
參與者 Participants：Peter, Tim, Yi-ting, Allison, Josh
![](https://g0v.hackmd.io/_uploads/HkkN9DAbfe.png)


https://www.vtaiwan.tw/jitsi
(請用Google登入以參與視訊並啟用轉錄功能)

::: success
## 專案儀表板：目前 vTaiwan 社群在進行的事情 
以下是 vTaiwan 目前正在進行的幾個不同的專案！如果有興趣，可以在取得共筆編輯權限後+1，或者是
1. [vTaiwan議題小聚](/GUe0KXMsQBC-6KTIUPBVnA)
    - 適合對審議、公共討論有興趣的人加入
    - 活動咖的最愛
    - 如果有公民團體與組織想要合作也可以看過來！
    - 目前也在進行 [AI 用於數位公民參與經驗](/nJUYukWFSWuEsG-XQalHcA) 的收集！
2. [vtaiwan x Tech for Good Asia 合作](/PdvNjkLlSd-WytXN_otWbQ) 計畫
    - 預計會討論人工智慧相關議題
    - 需要研究與分析人才
    - 想要貢獻議題或意見這邊請
3. [vTaiwan 網站改版，啟動！](/Q3_hgovyRHusEq8-3nomPQ)
    - 目標是打造新的 vTaiwan 網站
    - 結合 blog、媒體資源、活動發佈、建立線上活動與討論等功能
    - [vTaiwan 網站更新後的治理機制](/HxWF11d4Tz-AfHb3yjwQKg)也在建立中
    - 讓不會寫程式的人也可以加入貢獻者！
4. [vTaiwan 主視覺更新](/UqGh6pAUTwuNFr1RE21Rtw)
    - 目前已經設計完成！
    - 會發佈到新的網站，並在社群媒體上完成更新！
5. [Frankly 測試](/HVEdPVQqQHqVwvFECg7sRg)
    - 目前正在測試一個新的線上會議
6. 如果想要提案怎麼辦？
    - 目前的 vTaiwan 很歡迎與數位工具相關的公共討論！
- 如果想要成為協作者或者是貢獻者，我們正在努力讓說明更清楚，介面更友善！
:::
## 自我介紹
- Billy: 花蓮災象回波社群
- Josh: Researcher 
- Evgeniia: Russian in Texas, working on digital democracy 
- Tim: Engineer 
- Allison: High school student

## 小小的分享
### 近況更新：
- 上週有把 vTaiwan 電子報做出英文版了！
    - https://substack.com/home/post/p-200746798
- 最近在西班牙出差，仍然有持續在撰寫相關內容中

## TWNIC x vTaiwan 審議大會
- 改名了！避免參與者搞混。
- 第一場活動在 7/17 於 TWNIC 舉辦
- 目前活動規劃：https://docs.google.com/document/d/1jTHJ7KlLN0oBFdFZDbLdQVdr7wnnb91m9nBKpgFRfIM/edit?usp=sharing
- 在不同的活動環節會使用不同的 AI 工具
-
### 討論
- 產出的內容有哪些？
- 有哪些資料可以討論
- civic talk 作為審議資料的可能性
### 工作人員
- Peter 
- Yi Ting Lien 
- Tim 
- 來問問看 tofus 要不要來玩
- 來問問看 Ronny 作為參與者兼志工
- 來問問看 mashbean 
### 新的工具
- 將大家實際討論的內容直接轉換成種子意見：https://hear.atgora.org/
- 原理：利用 whisper 擷取大家的談話，分段去截取相關的意見
- 目前發現的問題：
    - 分段與精準度的問題
    - 可能要讓大家都可以協助編輯 / 調整
- 可以解決的問題：
    - 可以讓大家發言時，其他人可以針對意見提供回應
    - 可以解決大家手動輸入意見時，沒有辦法以口頭表達意見的問題

## Civic talk 
- 利用 AI 整理議題內容的實際案例
- https://ronnywang.github.io/civic-notes/cheerleading-male-gaze/ 
- 有些討論：
:::info
Tim: 
看起來像一些新聞網站會有的文章風格 (如 名詞解釋、重點整理)。
這可能是雙面刃：好處是符合台灣人平常的閱讀習慣。但也容易被視為另一個新聞媒體，要如何建立公信力與差異化也值得思考。

另外排版上有一點建議：【名詞說明】會介紹到的詞最好在前面【事情始末】先簡單出場使用。
像我初看到【名詞說明】的「男性凝視」跟「性別物化」介紹，較難直覺地跟議題前述爭點產生關連性，看不懂的情況下只想跳過說明直接閱讀下文，通篇閱讀的體驗就沒那麼順。

Johnson: 
文章風格 ai 感太重，論述少，但整體內容又太多 
> 不過我覺得如果是議題資訊整理的話，本來就是以資料匯集為主，這邊的期待對齊可以再調整一下。[name=peter]
:::

### Civic Talk 共筆
- 專案共筆：[vtaiwan 2026 新專案 civic talk](/u6Z7tvLKTzWYSH49YTCw1A)
- [civic talk 管理者指南](/2iS9plTURyuEb9U_vdN1qg)
- Github 連結：https://github.com/v-taiwan/civic-talk
- 目前部署連結：https://civic-talk.pages.dev
- 上週正式完成部署與實作
- 下一步：思考應用與宣傳，還有招募使用者
### Milestone: 
- 7/17@TWNIC 活動上可以使用

## 大翻譯運動
- 起因：看到歐洲議會研究處發了一篇71頁的報告，分析數位工具在公民參與中的應用。有很詳盡的分析，除了有整理世界各地的 94 個案例（包含工具與倡議外），也深入分析了11個案例，並且提出分析標準。
- 原文連結：https://www.europarl.europa.eu/RegData/etudes/STUD/2026/774753/EPRS_STU%282026%29774753_EN.pdf
- 翻譯文件連結：https://docs.google.com/document/d/1yrT1BKTJo2DMDhmRSWXwcBMdYS-A5yjaIQziA7vD1wg/edit?tab=t.0 
### 討論
- 報告內容仍然是不全的，要檢查與對應相關內容
- 
#### 將意見轉成 agent 的 skill 或資料庫
- 將市民的意見變成 AI 的 context
- 對應到不同機構的政策承接
#### Polis 當作是 wiki survey 的一部份
#### 11 個案例應該都是偏好一條龍的專案
- Decidim 

### 觀察到的問題
- 重複性
- 贅字的問題





## 轉錄逐字稿與AI大綱


### 轉錄逐字稿詳情

https://www.vtaiwan.tw/transcription_detail/20260610

### AI大綱

**2026‑06‑12 會議（崔家瑋、Tim、Joshua、Alison 等）重點整理**  

---

## 1. 活動概述
- **活動時間**：2024‑07‑17（星期五）  
- **形式**：線上／混合（可視訊、實體投影）研討會，主題聚焦 **AI 生成/散布不實資訊的治理**。  
- **名稱**：暫定為「AI‑Risk & Governance 小聚」或「AI 散布不實內容工作坊」。  

---

## 2. 主要目標 & 預期產出
| 目標 | 具體產出 |
|------|----------|
| 探討 AI 生成不實資訊的風險與因應 | - 產出問題圖譜（圖表、統計）<br>- 產出政策建議報告（PDF） |
| 測試與展示數位參與工具 | - 以 **Polis**、**SenseMaker**、**Mentimeter**、**CivicTalk** 為核心的實作演練<br>- 產出即時互動結果（投票、文字摘要） |
| 建立跨平台合作機制 | - 以 vTaiwan、TWNIC、VT1（vTaiwan+）等社群為合作聯盟<br>- 共同發布工具說明文件、貢獻者名錄 |
| 收集與分析參與者意見 | - 逐字稿＋AI 自動摘要（語序、內容微調）<br>- 分析參與者人口統計（年齡、地域）與意見分歧點 |

---

## 3. 使用工具概況
| 工具 | 用途 | 目前狀態 |
|------|------|----------|
| **Polis** | 大規模意見分群與共識圖譜 | 已邀請領域專家、待設定訪談|
| **SenseMaker** | 儲存與視覺化質性資料 | 已整合至工作流程 |
| **Mentimeter** | 實時投票、意見收集 | 需加入**編輯/審核**功能避免不完整意見 |
| **CivicTalk** (或 **Civit Talk**) | 生成 Prompt、AI 輔助立場說明 | 預計作為議題手冊整理工具 |
| **AI API**（如 OpenAI） | 文字自動摘要、意見微調 | 正在測試自動化逐字稿與即時補正 |

---

## 4. 議題與內容規劃
1. **AI 生成/散布不實資訊的風險**  
   - 探討技術原理、散布渠道、治理挑戰  
2. **案例分享**（由 Alison、翊婷等主持）  
   - 近期 AI 產出不實資訊的實例  
3. **意見證據收集**  
   - 前期問卷 → 文字/投票 → Mentimeter 分析  
4. **實務工具展示**  
   - Polis 互動示範、SenseMaker 資料探索、CivicTalk Prompt 產出  
5. **結果討論與政策建議**  
   - 以 **Mentimeter** 結果為基礎，產出圖表與報告  

---

## 5. 人員分工與參與狀況
| 人員 | 角色 | 目前狀態 |
|------|------|----------|
| **崔家瑋** | 主持、議題設計、進度追蹤 | 主要負責議題資料收集、與專家邀請、會前會安排 |
| **Tim** | 可能協助現場執行、志工支援 | 仍待確認是否能於 7/17 參與 |
| **Alison** | 內容翻譯、文件校稿、志工 | 已承諾協助翻譯與現場支援 |
| **翊婷** | 現場主持、意見蒐集 | 已確定出席 |
| **Joshua** | 工具開發與測試、技術支援 | 負責 Mentimeter、CivicTalk 功能調整、GitHub 回報 |
| **Ronnie**、**Edison** | 可能的志工或測試者 | 正在聯繫，視需求邀請參與 |
| **外部專家**（IWIN 前員工、性影像領域） | 提供領域知識、協助宣傳 | 已發出邀請，等待回覆 |

---

## 6. 時程與流程
| 時間點 | 工作項目 |
|--------|----------|
| **6/20‑6/30** | 完成議題資料彙整、邀請專家、確認工具測試環境 |
| **7/1‑7/7** | 發布前置問卷（Polis、Mentimeter），收集意見證據 |
| **7/8‑7/12** | 內部 **會前會**（模擬議程、測試工具） |
| **7/13‑7/15** | 最終Agenda確認、參與者邀請函發送、投影片與手冊製作 |
| **7/17** | 活動當天流程：<br>1️⃣ 開場（vTaiwan、TWNIC 介紹）<br>2️⃣ 主題分享（Alison、翊婷）<br>3️⃣ 工具示範（Polis、SenseMaker）<br>4️⃣ Mentimeter 投票與即時討論<br>5️⃣ 結果彙整、PDF 報告發布 |
| **7/18‑7/31** | 整理逐字稿、AI 摘要、產出正式報告、上傳至 GitHub & 共筆 |
| **8/20** | 針對 7/17 會議的回饋與工具優化（第二輪測試） |

---

## 7. 技術需求與功能調整
- **Mentimeter** 必須支援「意見編輯+審核」功能（admin 可微調文字、語序）。  
- **逐字稿自動化**：利用 AI API 把語音即時轉文字，並自動生成摘要與關鍵詞。  
- **多人麥克風**：為避免單一麥克風無法對應說話者，考慮在現場設置 2‑3 個收音點，或允許參與者自行上傳語音檔。  
- **權限管理**：使用 email 方式快速授權編輯者（避免 Zero‑Auth 複雜流程），兼顧資料安全。  
- **工具說明頁**：需加入合作團隊（VT1、vTaiwan）與貢獻者的 Credit，並同步至 GitHub README。  

---

**結語**  
會議在明確劃分目標、工具與人員責任後，已形成可行的活動藍圖。接下來的重點是 **確保工具功能（特別是 Mentimeter 編輯/審核）** 能在 7/13 前完成測試，並完成 **專家與志工的最終確認**，以保證 7/17 活動能順利執行、產出完整報告與後續可持續的工具資源。祝大家後續工作順利！  