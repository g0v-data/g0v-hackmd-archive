...
│                                               │
│   存放: chunk_id + embedding + 少量 metadata  │
│   只放「需要在 filter 階段用」的欄位          │
└──────────────────────────────────────────────┘
                      │
                      │ 透過 chunk_id 關聯
                      ▼
...
│                                               │
│   存放: chunk 原始內容、原始文件、附件        │
└──────────────────────────────────────────────┘
```

1. 各司其職
向量資料庫專注向量相似度搜尋，PostgreSQL 處理結構化查詢與 JOIN，物件儲存負責大內容。每個系統做自己擅長的事，效能與成本都最優。
2. 向量資料庫的 metadata filter 性能會掉
Qdrant、Weaviate 都支援 metadata filter，但 metadata 欄位多了之後 filter 效能會明顯退化。把高基數欄位放 PostgreSQL，向量資料庫只放 filter 時最常用的少數欄位（5-10 個）。
3. 演進靈活
換 embedding model 要 re-embed 整個向量資料庫，但 metadata 不用動。換 vector DB 也不影響 metadata。解耦的好處在三五年的維護週期中會非常明顯。
4. 權限決策必須走 SQL
EDA 的權限規則複雜（專案 × 團隊 × 機密等級），向量資料庫 filter DSL 表達不了。在 PostgreSQL 用 row-level security policy 才是合理做法。