---
tags: web3, Shoutout
---

# da0 Shoutout 02/20 會議記錄
* 時間：2024/02/20 20:30
* 參與者：昶惟、Chubby、Danny、Kay
* [會議連結](https://meet.google.com/tpx-cukc-mfn)
* [上次會議紀錄](https://g0v.hackmd.io/7DttJh8PR3qVK2gLE5FIvA?both)
* [SO Project Master Sheet](https://docs.google.com/spreadsheets/d/1TiLe2t5XUeXS1vE7gJmyeC8GRLaTwU0t_UstoFFrmaQ/edit#gid=1051831713)


## 重要時程
[2024 例會安排](https://docs.google.com/spreadsheets/d/1TiLe2t5XUeXS1vE7gJmyeC8GRLaTwU0t_UstoFFrmaQ/edit#gid=2098430858)

## 例會議程
* [功能設計/開發] 通訊版 SO & 前端展示頁面
* [宣傳推廣] SO 使用教學
* [專案管理] Server 費用優化
* [專案管理] 專案募資
* [專案管理] 專案激勵機制
* [專案管理] 專案開源
* [專案管理] 專案流程優化

## To-Dos Checklist


## 會議紀錄
* [功能設計/開發] 通訊版 SO & 前端展示頁面
    * Jack: 今天沒辦法線上參加，以開發角度跟目前所聽到過的需求來提供一些意見，現在基本需求我覺得有資源的話是可以開工： slack bot 紀錄 workspace/team 資料到資料庫、 line bot 初始架設、前端網頁架設(我聯絡不上 Matt，如果拿不到他那的程式碼可能就重寫這樣)。前面兩項我覺得目前蠻直觀的，前端要怎麼樣可能可以麻煩設計考量到我們想要分私有跟公共的部分來討論，或是前端也是可以先行做一個簡單的介面，我還有想清楚的比較是這個前端要靠什麼機制去驗證使用者看到私人組織的 SO (Slack ID, email)?
    * Labfront/da0/Fab DAO: user interview
* [宣傳推廣] SO 使用教學
* [專案管理] Server 費用優化
* [專案管理] 專案募資
* [專案管理] 專案激勵機制
* [專案管理] 專案開源
* [專案管理] 專案流程優化
    * Lucky
    * Kay
        * 邀請夥伴在某幾場例會做 SO 專案以外的分享
        * 大家如果有想從誰身上學到/聽到什麼，歡迎[填表](https://docs.google.com/spreadsheets/d/1787uStg1P0cyGzzkZz3gl5BMfmCTAZARxJpqjD28pPQ/edit#gid=0)，我來安排
    * 昶惟

## Action Items
* 進行 Labfront/FAB DAO/da0 用戶訪談（[Trello Task](https://trello.com/b/nF7sUC78/shoutout-2024))

## AI 整理總結


## Reference


## Meet開會文字頻道


## AI 逐字稿