---
tags: vTaiwan小松
---
# 20250212 vTaiwan 小黑客松
時間 Time ：19:00-20:00
地點 Location ：線上 Online
參與者 Participants: Peter, T, Josh, Allen
線上參與連結 / Link：https://meet.jit.si/vtaiwan
![](https://g0v.hackmd.io/_uploads/r1vHUyHR1e.png)



## 自我介紹與新手導覽
- [新手簡報](https://docs.google.com/presentation/d/1ELAVIpaPVCmAx7nq-7e8-SVrckZ4ohRwn3V-TSpe78U/edit?usp=sharing)

## 小小的分享
- 2/25 見面討論
- [Sensemaking 討論](/TDZybeTlR5iVwicR3f-x5w)

### Allen 上次小松後小小測試的結果
![](https://g0v.hackmd.io/_uploads/HJcqF-5YJl.png)
- methodology: 
    - 先將逐字稿轉成 pdf 檔案
    - 餵進去 KLM
    - 若要生成中文內容，在 url 加上`?hl=zh-TW`
    - [URL](https://notebooklm.google.com/notebook/f2e1ddd8-87c2-4217-9f15-87a6b661e14c?hl=zh-TW)
- 推薦大家利用檔案去試試看
- 問題：
    - 有辦法讀語音檔案嗎？[name=T]
    - 可以丟音訊檔
        - 可以
- sensemaking 有做到跑出 unit test，但不確定如何繼續
    
## 1220 活動
- [共編文件](https://docs.google.com/document/d/1kYIYbHdTctGBXrV-U3qivkzByV-uAxOAiS7fB_sGacU/edit?usp=sharing)
- 感謝 T 成功生產出文字紀錄檔！
- [文字紀錄檔 Part1](https://asr.yating.tw/file?eid=ZLJU8u9jSOj+LYZWlgaoRSWgzkvcQcOaGA2m+qFiFR9bZzJbVLPgKABzqE5Eqoa6) 
    - [Google 共編](https://docs.google.com/spreadsheets/d/122ZKlvLuGaOLqs-4FXb4DjI5cww_MmZOafBsuli2WJU/edit?usp=sharing)
- [文字紀錄檔 Part2](https://asr.yating.tw/file?eid=OjofuTdhBPdlDnoF8+dUJteMHbKrinTGKt8J/IIwae/DD+21ZhRKd++BPY7tFY9W)
    - [Google 共編](https://docs.google.com/spreadsheets/d/1Wm8hL8ZnjfYvuoq2eFYOW7Nuqr4qpR2UplBhBnYLplQ/edit?usp=sharing)
- 以下是從原始錄影分離出來的[語音記錄檔](https://drive.google.com/drive/folders/1ymAbB8dqpweVd1W6ROlmW3GvurZ5ffTM?usp=sharing)
- 上傳到雅婷只要幾分鐘就能跑完解析，大家也可以自行玩玩看，或是拿去測試其他轉錄工具~
- ps. 已參考上次小松會議記錄的「轉錄工具」PPT，但 Good Type 免費版僅有每月 1.5 小時額度，所以不適用這次議題小聚。
- [紀錄文件](https://docs.google.com/presentation/d/1bXoyr-b0jjirVQ4NgCn6J5-6QDOXibL8ktCmiqyVza8/edit#slide=id.p)
### 待辦事項
- Part1+2 總共50,000字，招募大家來清理
    - 約一個時間來整理逐字稿
        - 大松當天下午
    - 紀錄部分會是挑戰
    - 
- 逐字稿的整理要到什麼程度
- 
### 上傳到 github 上了
- 目前 mentimeter 的結果已經上傳到 github 上了！[看結果](https://github.com/v-taiwan/241220-AI-Regulation)

## RightsCon 相關 side event
- 目前 2/25 有一個 meetup w/ Chatham House 
- 來自 slack 上的訊息，感覺可以聯繫一下
:::info
Hi all! I'm Emily, a user experience researcher at Jigsaw, a small unit within Google that conducts research and builds tools for the common good. My current research focuses on ways to improve online conversations and tools for scaling and making sense of public input from deliberations/deliberations, for example in our open sources sensemaking library, which vTaiwan leaders like Audrey Tang and others in the community have been experimenting with, as well as Polis (hi to 
@dzn
 who recommended this space!) https://github.com/Jigsaw-Code/sensemaking-tools/
I'll be in Taipei at the end of the month for RightsCon and hoping to connect with some folks in the community while there. Will any of you all be there, or have any events/happenings to recommend? Thank you!
:::
- 有沒有想問 sensemaking 的問題或討論～
### Sensemaking material
- intro:
    - sensemaking 由 jigsaw 所開發，是 Google 底下的團隊
    - 有一兩個人之前在 polis 團隊
- [Making Sense of Large-Scale Online Conversations](https://medium.com/jigsaw/making-sense-of-large-scale-online-conversations-b153340bda55)
- 最近發布的[報告:Exploring How AI Might Enhance Our Digital Public Squares](https://medium.com/jigsaw/exploring-how-ai-might-enhance-our-digital-public-squares-ec60eb702887)
- [Demo](https://internal-makeorg-sensemaking-paris.cip.org/sensemaking)
    - 概念上就是有polis的報告但是你可以跟這份報告互動 [name=josh]
 
### Discussion Agenda 
-  [name=T]、[name=josh] 有問題可以提在這邊
-  轉換逐字稿的品質
    -  可以測試看看中文資料的狀態

### 合作可能性
- 接下來兩次的議題小聚可以合作 [name=josh]
    - 要確認語言、資料的類型問題 [name=peter]
- 12/20 議題小聚的分析報告
- Action List
    -用英文版簡單介紹一下 12/20 議題小聚的方法論與分析結果

## 2/22 大松活動
- [報名連結](https://kktix.com/events/g0v-hackath65n/registrations/new)
- 出席
    - peter
    - 宇亭
- 有先搶下一個提案，可以看看想要測試什麼東西
- 宇亭提議
    - 延續 vTaiwan 的101, FAQ, 邀請對 vTaiwan 有興趣的人來幫我們看看要新增哪些問題和回答。從新手的觀點獲取需求，完善文件。
        - action list: 列出問題
        - 
    - 不管從工程師的角度思考（不要重複造輪子）或從研究者的角度思考（奠基於既有文獻發展研究），我覺得我們關注的議題在過去一定有很多人討論、嘗試過。剛好這次在台大辦，網路上支援獲得各種學術文獻。我自己覺得可以邀請大家幫忙蒐集文獻和建立筆記，讓 vTaiwan 社群的決策是基於科學實證的，在論述上會比較站得住腳。這部分我可以負責建立模板和 workflow，讓所有人都可以輕易上手。
        - 或許可以更進一步討論利用 AI 的可能性
        - 模板與 workflow 可以麻煩 [name=yuting]
    - 試試看用 Argdown 來做討論的場合
