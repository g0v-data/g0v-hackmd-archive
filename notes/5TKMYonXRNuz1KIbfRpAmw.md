...
- 這兩天看起來逐字稿品質還不錯，沒有感受到品質有改變 [name=bil]
...
api: https://github.com/cofacts/rumors-api/pull/381
db: https://github.com/cofacts/rumors-db/pull/77

TODO: 
1. document detail migration steps
2. cluster mode?
3. 

generated by ai
```
🔴 高風險：Migration 步驟不夠具體
PR body 的 Migration / Deployment Notes 只有三行：
> 1. Before deploying: Run the reindex migration script in rumors-db
> 2. rumors-db PR must be merged first
> 3. Zero-downtime strategy: The reindex script creates new v9-compatible indices while old ones remain operational. Switch aliases after verification.
問題：
1. 「Run the reindex migration script」具體怎麼跑？ rumors-db PR 有給 SOURCE_HOST / DEST_HOST 的範例，但 rumors-api PR 沒有引用。一個不熟的人看到這三行，不知道要去 rumors-db PR 裡找指令。
2. 部署順序不明確： 是先升 ES 9.2 cluster → 跑 reindex → 再部署新版 rumors-api？還是可以先部署 rumors-api？需要一個 step-by-step runbook。
3. 「Switch aliases after verification」 — 怎麼 verify？要確認什麼？document count 一致？API smoke test？沒有具體的 verification checklist。
4. Rollback plan 完全沒提。 如果 migration 跑到一半出問題，如何回退？
5. production ES cluster 的 reindex.remote.whitelist 設定 — docker-compose 有註解掉的這行，但沒在 deployment notes 提到 production 需要設。
🟡 中風險：環境前置條件散落各處
- Node 24 upgrade 是和 ES 9 綁在同一個 PR。如果執行者的環境還在 Node 18，npm install 會直接失敗嗎？還是能裝完？package.json 只寫 >=18，但 package-lock.json 的 lockfileVersion 3 需要 npm 7+（Node 16+），所以理論上能裝，但 runtime 可能有問題。這沒有被明確說明。
- docker-compose 的 volume 從 esdata 改成 esdata-v9 — 對已有開發環境的人來說，需要 mkdir esdata-v9。README 的 first-time setup 仍寫 mkdir esdata。
- service name 從 db 改成 db-v9 — 任何依賴 docker-compose exec db 的人/腳本會壞。
🟡 中風險：Companion PR 的依賴關係
- rumors-db PR #77 的 .gitmodules 改了 URL (MrOrz/rumors-db → cofacts/rumors-db)，但 submodule 指向的是 feature branch feature/upgrade-to-elasticsearch-v9，不是 main。Merge 完後，submodule pointer 需要指向 merge 後的 main commit 嗎？還是可以直接 merge？這個時序問題沒有被交代。
```
