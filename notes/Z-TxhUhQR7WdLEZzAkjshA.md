:::danger
展頌股份有限公司會議專用紀錄(僅顯示網址，登入密碼請洽承辦人員)
本頁快速網址：[https://bit.ly/chainlonmeet](https://g0v.hackmd.io/Z-TxhUhQR7WdLEZzAkjshA)
:::
:::warning
目錄
[TOC]
:::
#### Google Meet(需要有Google帳號，中國不通)
(展頌Google Meet會議室)https://meet.google.com/znp-xmiy-hyj
![](https://i.imgur.com/bIgZSUY.png)
(展頌Google01)https://meet.google.com/yes-mmtu-tbw
(黃吉祿)https://meet.google.com/cup-xvnz-uog
(羅堂國)https://meet.google.com/jdq-kkah-wvu
(斗三廠)https://meet.google.com/ctb-jjiu-ept

#### CISCO(參加，不是登入，不須安裝，記得輸入暱稱)
:bulb:蘋果系統(iPhone、Apple桌機)請用Google Chrome、Safari
Windows桌機或Android可用App或瀏覽器(Edge,Chrome,Firefox確認可用)
展頌CISCO會議室(手動輸入會議ID) https://mt.chainlon.net/
[展頌CISCO會議室1(會議ID：22927617)](https://mt.chainlon.net/invited.sf?secret=FSqW0vWl.lz8tQL1qCE4zg&id=22927617)
[展頌CISCO會議室2(會議ID：chainlonit)](https://mt.chainlon.net/invited.sf?secret=3R8kejuzQCRs2ZsoziFbKg&id=chainlonit)

#### 視訊設備自我測試
https://www.toolskk.com/webcam-mic-test

#### Microsoft Teams(桌機可用瀏覽器免安裝，手機要安裝，都需要帳號，2025/3/27更新)
展頌台中(管理員：chainlon01@outlook.com)：尚未重新設定
展頌資訊專用(管理員：Ken)：https://teams.live.com/l/invite/FEAO5Ql1B_S9VJ5kQQ

#### Zoom(會議ID似乎會變，2023/7/3 更新)
會議 ID：621 940 4586
密碼：22927617
以瀏覽器加入時會要求你安裝並自動下載軟體可忽略之後改為選擇使用瀏覽器就不須安裝，每次會議都會改ID且無法自訂密碼，請在會議前聯繫、取得主持人的ID、密碼
https://us05web.zoom.us/j/6219404586?pwd=TEUxS1FnVE0xVUlMVUVUS2IyYXBGdz09