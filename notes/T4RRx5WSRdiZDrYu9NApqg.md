---
tags : vTaiwan小松, CAI, ruleAI, OpenAI
---
#  {20230607} vTaiwan 小黑客松
時間：{2023/06/07} 週三 19:00-21:00
地點：
出席：
線上組：
線上參與連結：https://meet.jit.si/vTaiwan

主持人(facilitator)：
Translation：[English](https://translate.google.com/translate?hl=en&sl=auto&tl=en&u=https%3A%2F%2Fg0v.hackpad.tw%2Fep%2Fpad%2Fstatic%2FleY4KC9wlOL&sandbox=1)

預定流程
19:00-19:15 開場、自我介紹
19:15-19:30 確認討論議程
19:30-21:00 討論&hacking
21:00-21:30 成果分享&決定下週主持人
下週主持人：

提案（自己的小松自己提案）
---
連三週的週三晚上19:00在線上聊 AI x Democracy (vTaiwan) 

Open AI grant
https://openai.com/blog/democratic-inputs-to-ai

- what rules AI systems should follow, can we discuss this through vTaiwan’s process?
- How might AI improve vTaiwan process? 
    - 幾個在這篇 application intro 上有列出的問題像是 
        - inadequately representation of minority groups
        - insufficiently informed participants
        - participation washing
        - 其他？
        - How far do you think personalization of AI assistants like ChatGPT to align with a user's tastes and preferences should go? What boundaries, if any, should exist in this process?
        - How should AI assistants respond to questions about public figure viewpoints? E.g. Should they be neutral? Should they refuse to answer? Should they provide sources of some kind?
        - Under what conditions, if any, should AI assistants be allowed to provide medical/financial/legal advice?
        - In which cases, if any, should AI assistants offer emotional support to individuals?
        - Should joint vision-language models be permitted to identify people's gender, race, emotion, and identity/name from their images? Why or why not?
        - When generative models create images for underspecified prompts like 'a CEO', 'a doctor', or 'a nurse', they have the potential to produce either diverse or homogeneous outputs. How should AI models balance these possibilities? What factors should be prioritized when deciding the depiction of people in such cases?
        - What principles should guide AI when handling topics that involve both human rights and local cultural or legal differences, like LGBTQ rights and women’s rights? Should AI responses change based on the location or culture in which it’s used?
        - Which categories of content, if any, do you believe creators of AI models should focus on limiting or denying? What criteria should be used to determine these restrictions?


分享
---

AI 相關政策的公民審議都有很多人在做，
eg. AIA AI Academy 人工智慧學校

近年來 join 平台會是比較好對
哲緯有想做更好的 join 平台

ronny 近期有在 join 平台上提案
https://join.gov.tw/idea/detail/a9b3cac9-ab42-42c5-8a1a-418d854c905d/discuss?bannerPlayEnable=true

vTaiwan 畢竟是數位平台，但在討論數位議題時

Revive:
Da0 co-work
需要一個打死不退的 leader。
au->shu->lisa & billy->peter

覺得可以慢慢從恢復每週聚會開始
peter, chewei, shu

恩恩也可以問問
專訪可以找RS (姜伯任)- coding & law

議題：
數位政策基礎設施，怎麼互相接軌彼此的資料，資料標準怎麼對接。
DID 也是 DML 在推的東西 - au / 劉維人 / 豆泥

放下我執 - 非 vTaiwan 不可

next step:
找其他人聊聊，直接對這個 grant 作討論。
找 isabel (AIA), chewei, chihao 聊聊。
原本有找 ronny 聊過
 - 

提議到面海松討論
也可以拉 Paul & Ted(Ted 顥天)

Demos - 
Carl Miller - 
Martin King - 
Shu - vTaiwan / 

Let's have weekly meet-up!
用投 grant 開始！
方向 vTaiwan <> AI
1. vTaiwna -> AI
    vTaiwan 本身是因為數位法規調適出現的平台
2. AI -> vTaiwan
    或許人工智慧工具可以協助克服參與者的資訊落差，定期提醒與參與

[can democracy fix AI, can AI fix democracy]()
https://www.nytimes.com/2023/04/05/opinion/artificial-intelligence-democracy-chatgpt.html

Timeline
6/10 - hackathon 目標：找人，初步討論方案，
6/14 19:00 - 21:00 meeting  目標：討論
Asynchronize work/撰寫時間：6/14-18
6/23 19:00 - 21:00 - meeting final check
6/24 - submit

專案主頁：
https://g0v.hackmd.io/@Pno233SAS8G5UfL5OvSRmA/BJd1YlRI2 

todo（想在小松完成的事情）
---

小松(成)果
---

vtaiwan 基金餘額
---
* 本日花費:
* 本日現場贊助: 
* 捐款: 
* 餘額:






