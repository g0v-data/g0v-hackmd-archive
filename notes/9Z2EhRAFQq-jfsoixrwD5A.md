---
title: SaaS 韌性產品 self-host guide
tags: digital-resilience, resilience, internet-shutdown, digiresi, civil-defense, 民防, 數位韌性松, DigiResiTh0n, hackathon, civil defense,
---

# SaaS/AI 產品 self-host 韌性 ++ guide

## SaaS
- [遠離大手會社，自架基本服務](https://pretalx.coscup.org/coscup-2023/talk/KBT8C7/)
    - 你總是使用 GMail, Google Photos, Google Map, Google Drive 等等 Google 全家桶嗎？擁抱開源文化、卻又不得不將各種個資、隱私、行為剩餘送給大手會社們？懷念撥接 BBS 時代的美好、卻擔心自架沒有 interoperability?
    - 本議程以實務角度出發，完全使用開源套件，提供各種自架服務的小技巧。自己動手、豐食足食。
    - 本議程將探討以下新手小技巧:
        - 樹莓派、Digital Ocean、還是便宜二手 x86
        - 固定 IP vs. 浮動 IP
        - 現代生活的必需品: Let's Encrypt
        - Email -- Postfix + CA + Dovecat 還有 GMail 的小鎖頭
        - 隱私的小幫手: wildcard email
        - Blog: 不用 blogger 也不用 CMS 的懶人法
        - 行事曆與聯絡人
        - 照片的雲端備份
        - 未竟之業: 貓臉辨識
    - [slide](https://miaoski.github.io/coscup-2023.html#1)
    - [session video](https://www.youtube.com/watch?v=TMhONJPJQ5g)

- [如何逃離 Google 之一](http://miaoski.github.io/escape-from-google-1.html)

## AI
- [LM Studio](https://lmstudio.ai/)
    - With LM Studio, you can ...
    - 🤖 Run LLMs on your laptop, entirely offline
    - 👾 Use models through the in-app Chat UI or an OpenAI compatible local server
    - 📂 Download any compatible model files from HuggingFace 🤗 repositories
    - 🔭 Discover new & noteworthy LLMs in the app's home page