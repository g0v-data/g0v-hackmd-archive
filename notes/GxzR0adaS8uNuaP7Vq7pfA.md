...
### Admin API settings on Cloudflare Zero Trust
...
- `(dev-)admin-api.cofacts.tw`: Allow all users & any service tokens
  ...
  - Can use any service token to get `/openapi.json` so that 
- `(dev-)admin-api.cofacts.tw/`


#### Access groups

People & token that can access application
 

> https://developers.cloudflare.com/cloudflare-one/policies/access/app-paths/#policy-inheritance
